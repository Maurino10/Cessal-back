-- ------------------------------------- Treatment Temp Tpi
    CREATE OR REPLACE FUNCTION treatment_temp_tpi()
    RETURNS void AS $$
    BEGIN
        INSERT INTO province (name) 
            SELECT DISTINCT (province)
                province
            FROM temp_tpi ORDER BY province;
        ...
        TRUNCATE TABLE temp_tpi; 
        ALTER SEQUENCE temp_tpi_id_seq RESTART WITH 1;
    END;
    $$ LANGUAGE plpgsql;


-- ------------------------------------- Trigger User Inscription Approved 
    CREATE OR REPLACE FUNCTION user_inscription_approved()
    RETURNS TRIGGER AS $$
    DECLARE
        profil_id VARCHAR;
    BEGIN
    ...
    CREATE OR REPLACE TRIGGER trigger_user_inscription_approved
    AFTER UPDATE ON inscription
    FOR EACH ROW
    EXECUTE FUNCTION user_inscription_approved();
...
-- ------------------------------------- DROP SEQUENCE 
    ...
    DROP SEQUENCE temp_tpi_id_seq CASCADE;               
